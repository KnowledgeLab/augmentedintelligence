...
def makeNYTplots(nytimes_model, dimensions, words):
    def makeDF(word_list):
        dims = {name : [] for name in dimensions.keys()}
        for word in word_list:
            for name, dim in dimensions.items():
                dims[name].append(sklearn.metrics.pairwise.cosine_similarity(nytimes_model[word].reshape(1,-1), dim.reshape(1,-1))[0][0])
        df = pandas.DataFrame(dims, index = word_list)
        return df

    def PlotDimension(ax, df, dim, nameW):
        ax.set_frame_on(False)
        ax.set_title('{} onto {}'.format(nameW, dim), fontsize = 16)
    ...
    dfs = {name : makeDF(w) for name, w in words.items()}

    fig, axarr = plt.subplots(len(dfs), len(dimensions), figsize = (16,10))

    for i, (nameW, df) in enumerate(dfs.items()):
        for j, (name, dim) in enumerate(dimensions.items()):
            ax = axarr[j, i]
            PlotDimension(ax, df, name, nameW)
    plt.show()
    plt.close()
